...
var template = Object.freeze({
  ...
  function filterBy(arr, search, delimiter) {
    ...
    for (var i = 0, l = arr.length; i < l; i++) {
      ...
      if (j) {
        while (j--) {
          key = keys[j];
            //にまび むげんのリストをしていなkeyにfilterする
          console.log(contains$2(item,key, search));
          if (key === '$key' && contains$1(item.$key, search) || contains$2(item,key, search)) {
            res.push(item);
            break;
          }
        }
   ...
   * @param {String} search
   */
  function contains$2(val,key, search) {
    var i;
    if (isPlainObject(val)) {
      var keys = Object.keys(val);
      i = keys.length;
      while (i--) {
      	if (keys[i]==key) {
      		return getPath(val,key).toString().toLowerCase().indexOf(search) > -1;
      	}else
      	{
      		if (isPlainObject(val[keys[i]])||isArray(val[keys[i]]))
      		{

      			if (contains$2(val[keys[i]],key, search)) {
      				return true;
      			}
      ...
      i = val.length;
      while (i--) {
        if (contains$2(val[i],key, search)) {
          return true;
        }
      }
    } else if (val != null) {
